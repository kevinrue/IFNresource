...
initial <- list(
  FeatureAssayPlot(
    ...
    XAxisFeatureName = "ENSG00000000003",
    XAxisFeatureSource = "---",
    XAxisFeatureDynamicSource = FALSE,
    YAxisFeatureName = "lnc-MX1-4:3",
    YAxisFeatureSource = "---",
...
intro <-
  c(
    intro,
    "This panel can be used to view the counts for an individual lncRNA across different samples, for each donor A, B and C."
  )
...
intro <-
  c(
    intro,
    "Select the lncRNA you would like to view from the dropdown list under Y-axis feature in the <font color=\"#402ee8\">Data parameters</font> menu<br/><br/>"
  )
...
intro <-
  c(
    intro,
    "This panel depicts the expression of a selection of lncRNAs in a heatmap, for example, here we are looking at the expression of the top two up- and down-regulated lncRNAs. <br/><br/>Each column represents a sample and each row a lncRNA. Samples are ordered by treatment and donor."
  )
...
intro <-
  c(intro,
    "Rows can be clustered by checking the box under the <font color=\"#402ee8\">Data parameters</font> header.<br/><br/> lncRNAs of interest are entered by clicking the 'Edit feature names' box.")
